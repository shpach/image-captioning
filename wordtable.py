...
class WordTable():
    ...
    def save(self):
        ...
        pickle.dump([self.word2vec, self.sentence_dictionary, self.word_freq, self.num_words, self.idx2word, self.word2idx], open(self.save_file, 'wb'), protocol=4)

    def load(self):
        """ Load the word table from pickle """
        self.word2vec, self.sentence_dictionary, self.word_freq, self.num_words, self.idx2word, self.word2idx = pickle.load(open(self.save_file, 'rb'))
